...
func TestIntegrationExposureAttribution(t *testing.T) {
	...
	fClick := click("r2", "f", 12, "c3", at.Add(70*time.Second))
	fClick.ContentRef = fClick.WithVersion("v1")
	clicks := []Signal{
	...
	other := click("r1", "a", 2, "c1", at)
	other.TenantID = "other"
	if err := st.RecordSignals(ctx, "other", []Signal{other}); err != nil {
		t.Fatal(err)
	}
	...
	if r2 := rendered.Renders[1]; len(r2.Shown) != 2 || r2.Shown[1].Version() != "v1" || ids(r2.Clicks)[0] != "f:c3" || !r2.Clicks[0].Exposed || r2.Clicks[0].Position != 12 || r2.Clicks[0].Version() != "v1" {
		t.Fatalf("paginated render keeps absolute positions and version references: %+v", r2)
	}
	...
	if err != nil || len(foreign.Renders) != 0 || len(foreign.Unattributed) != 1 || foreign.Unattributed[0].TenantID != "other" {
