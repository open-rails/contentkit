import type { UploadUiMessageBundle } from "../i18n/messages.js";

export const ko: UploadUiMessageBundle = {
  common: {
    cancel: "취소",
    close: "닫기",
    save: "저장",
    saving: "저장 중…",
    retry: "다시 시도",
    change: "변경",
    upload: "업로드",
    editCrop: "자르기 편집",
  },
  crop: {
    title: "이미지 자르기",
    avatarTitle: "아바타 자르기",
    coverTitle: "커버 이미지 자르기",
    description: "드래그해서 위치를 조정하세요. 스크롤, 핀치 또는 슬라이더로 확대/축소할 수 있습니다.",
    area: "자르기 영역. 화살표 키로 이미지를 이동하고 + 또는 −로 확대/축소, R로 회전하세요.",
    zoom: "확대/축소",
    zoomIn: "확대",
    zoomOut: "축소",
    rotateLeft: "왼쪽으로 회전",
    rotateRight: "오른쪽으로 회전",
    reset: "초기화",
    undersized: "이 자르기 영역의 너비는 {width}px입니다. 고밀도 화면에서 선명하게 보이려면 {target}px 이상이 좋습니다.",
    tooSmall: "이 이미지의 너비는 최대 {width}px입니다. 최소 {min}px이어야 합니다. 더 큰 이미지를 사용하세요.",
  },
  avatar: {
    label: "아바타",
    empty: "아바타 없음",
    hint: "정사각형 이미지, {size}px 이상.",
  },
  cover: {
    label: "커버 이미지",
    empty: "커버 이미지 없음",
    hint: "가로로 긴 이미지, {width} × {height}px 이상.",
    drop: "여기에 이미지를 놓으세요",
  },
  poster: {
    title: "커버 설정",
    description: "동영상의 프레임을 고르거나 이미지를 업로드하세요. 커버는 동영상과 같은 비율입니다.",
    frame: "동영상 프레임",
    upload: "이미지 업로드",
    time: "시간",
    previousFrame: "이전 프레임",
    nextFrame: "다음 프레임",
    jumpTo: "{time}(으)로 이동",
    useFrame: "이 프레임 사용",
    crop: "자르기…",
    cropTitle: "커버 자르기",
    auto: "자동",
    autoHint: "직접 고르기 전까지 자동으로 선택된 프레임이 사용됩니다.",
    drop: "여기에 이미지를 놓거나 클릭해서 선택하세요",
    processing: "동영상을 아직 처리 중입니다. 인코딩이 끝나면 커버를 설정할 수 있습니다.",
    rendering: "커버 생성 중…",
  },
  preview: {
    title: "호버 미리보기",
    description: "동영상에 마우스를 올리면 소리 없이 재생될 구간을 고르세요({min}–{max}초).",
    section: "구간",
    start: "{time}부터",
    length: "{seconds}초",
    approximate: "대략적인 미리보기",
    rendered: "생성된 미리보기",
    save: "미리보기 저장",
    auto: "자동",
    rendering: "미리보기 생성 중…",
  },
  progress: {
    decoding: "이미지 여는 중…",
    hashing: "준비 중…",
    uploading: "업로드 중… {percent}%",
    completing: "업로드 마무리 중…",
    rendering: "크기별 이미지 생성 중…",
  },
  encode: {
    processing: "동영상 처리 중",
    queued: "처리 대기 중",
    queuedPosition: "대기 중 · {position}번째",
    stalled: "처리가 일시 중지되었습니다. 서버를 기다리는 중…",
    segments: "세그먼트 {done} / {total}",
    remainingSeconds: "약 {seconds}초 남음",
    remainingMinutes: "약 {minutes}분 남음",
    remainingHours: "약 {hours}시간 {minutes}분 남음",
    almostDone: "거의 완료",
    estimating: "남은 시간 계산 중…",
    phase: {
      queued: "대기 중",
      downloading: "준비 중",
      probing: "분석 중",
      encoding: "인코딩 중",
      muxing: "패키징 중",
      uploading: "저장 중",
      publishing: "게시 중",
      images: "커버와 미리보기 만드는 중",
    },
  },
  gallery: {
    label: "미디어",
    view: "보기",
    carousel: "캐러셀",
    grid: "그리드",
    counter: "{current} / {total}",
    slide: "{total}개 중 {current}번째",
    previous: "이전",
    next: "다음",
    goTo: "{index}번째 항목으로 이동",
    open: "{total}개 중 {index}번째 항목 열기",
    lightbox: "미디어 뷰어",
    lockedOne: "1개 항목이 더 잠겨 있습니다",
    lockedMany: "{count}개 항목이 더 잠겨 있습니다",
    processingImage: "이미지 처리 중…",
    image: "이미지 {index}",
    video: "동영상 {index}",
    failedImage: "이 이미지를 처리할 수 없습니다.",
  },
  player: {
    play: "동영상 재생",
    loading: "동영상 불러오는 중…",
    retry: "다시 시도",
    code: "코드 {code}",
    quality: "화질",
    auto: "자동",
    autoCurrent: "자동({quality})",
    failed: "이 동영상을 처리하지 못했습니다.",
    errors: {
      network: "동영상 서버에 연결할 수 없습니다.",
      access: "이 동영상에 더 이상 접근할 수 없습니다.",
      not_found: "이 동영상을 사용할 수 없습니다.",
      rate_limited: "요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.",
      unsupported: "이 브라우저에서는 이 동영상을 재생할 수 없습니다.",
      stalled: "재생이 멈췄습니다. 연결을 확인하고 다시 시도해 주세요.",
      unknown: "지금은 이 동영상을 재생할 수 없습니다.",
    },
  },
  errors: {
    generic: "서버에서 문제가 발생했습니다. 다시 시도하세요.",
    network: "연결이 끊어졌습니다. 네트워크를 확인하고 다시 시도해 주세요.",
    storage: "업로드가 중단되었습니다. 다시 시도해 주세요.",
    decode: "이 파일은 이미지로 열 수 없습니다.",
    invalid_request: "이미지를 저장할 수 없습니다.",
    unauthorized: "세션이 만료되었습니다. 다시 로그인해 주세요.",
    forbidden: "이 이미지를 변경할 권한이 없습니다.",
    not_found: "이 항목은 더 이상 존재하지 않습니다.",
    conflict: "다른 곳에서 이미지가 변경되었습니다. 새로고침 후 다시 시도해 주세요.",
    incomplete: "업로드가 완료되지 않았습니다. 다시 시도해 주세요.",
    not_uploaded: "업로드가 만료되었습니다. 이미지를 다시 선택해 주세요.",
    too_large: "파일이 너무 큽니다.",
    quota_exceeded: "저장 공간 한도에 도달했습니다.",
    type_not_allowed: "이 파일 형식은 여기에서 지원되지 않습니다.",
    checksum_mismatch: "업로드 중 파일이 변경되었습니다. 다시 시도해 주세요.",
    rate_limited: "업로드가 너무 많습니다. {seconds}초 후에 다시 시도해 주세요.",
    internal_error: "서버에서 문제가 발생했습니다. 다시 시도하세요.",
    allowedTypes: "{allowed} 형식을 사용하세요.",
    tooLargeBy: "이 파일은 {size}입니다. 제한은 {max}입니다.",
    image_too_small: "자른 이미지의 너비는 {width}px입니다. 최소 {min}px이어야 합니다. 더 넓은 영역을 선택하거나 더 큰 이미지를 사용하세요.",
    image_too_large: "이 이미지는 {width}×{height}px입니다. 이미지는 최대 {megapixels}메가픽셀까지 가능합니다.",
    image_unreadable: "이 파일은 올바른 {format} 이미지가 아닙니다.",
    render_timeout: "서버에서 아직 처리 중입니다. 준비되면 표시됩니다. 잠시 후 다시 확인하세요.",
    animation_not_allowed: "여기에는 움직이는 이미지를 사용할 수 없습니다. 정지 이미지를 업로드하세요.",
    animation_unsupported: "움직이는 {format} 이미지는 지원되지 않습니다. 움직이는 WebP 또는 GIF를 사용하세요.",
    animationFrames: "애니메이션은 최대 {maxFrames}프레임까지 가능합니다. 이 이미지는 {frames}프레임입니다.",
    animationSeconds: "애니메이션은 최대 {maxSeconds}초까지 가능합니다.",
    animationTooLarge: "이 애니메이션이 너무 큽니다: {width}×{height}px, {frames}프레임. 프레임 수나 크기를 줄이세요.",
  },
};
