...
func TestClientSearch_Integration_Keyword(t *testing.T) {
	...
	if len(trace.Sources) != 1 || trace.Sources[0].Backend != BackendKeyword || trace.Sources[0].Status != SourceStatusSucceeded || len(trace.Results) != 2 || trace.Results[0].ScoreKind != ScoreKeywordMatch || trace.Results[0].Key.ContentID != "1" {
