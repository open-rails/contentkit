...
func TestImportedTimestampsValidateWithoutDatabase(t *testing.T) {
	...
	}{{"tag", nil, nil}, {"tag", &zero, nil}, {"tag", nil, &zero}, {"", &valid, nil}} {
...
func TestImportedTimestampsPreserveChronologyAndTransactions(t *testing.T) {
	...
	for _, id := range []TaxonomyID{"import-a", "import-b"} {
	...
	check(s, "import-a", created, updated)
	changed := updated.Add(time.Hour)
	if err := s.SetImportedTimestamps(ctx, "import-a", nil, &changed); err != nil {
		t.Fatal(err)
	}
	check(s, "import-a", created, changed)
	if err := s.SetImportedTimestamps(ctx, "foreign-only", &created, &updated); !errors.Is(err, ErrNotFound) {
		t.Fatalf("foreign tenant lookup: %v", err)
	}
	if err := s.SetImportedTimestamps(ctx, "missing", &created, &updated); !errors.Is(err, ErrNotFound) {
		t.Fatalf("missing lookup: %v", err)
	}
	foreign, err := other.Node(ctx, "foreign-only")
	...
	if _, err := pool.Exec(ctx, fmt.Sprintf("DELETE FROM %s.content_search_dirty WHERE tenant_id=$1 AND content_id='import-b'", schema), tenant); err != nil {
		t.Fatal(err)
	}
	...
	if err := borrowed.SetImportedTimestamps(ctx, "import-b", nil, &changed); err != nil {
		t.Fatal(err)
	}
	check(borrowed, "import-b", created, changed)
	var dirty int
	if err := tx.QueryRowContext(ctx, fmt.Sprintf("SELECT count(*) FROM %s.content_search_dirty WHERE tenant_id=$1 AND content_id='import-b'", schema), tenant).Scan(&dirty); err != nil || dirty != 2 {
		t.Fatalf("import invalidation: %d %v", dirty, err)
	}
	if err := tx.Rollback(); err != nil {
		t.Fatal(err)
	}
	check(s, "import-b", created, updated)
	if err := pool.QueryRow(ctx, fmt.Sprintf("SELECT count(*) FROM %s.content_search_dirty WHERE tenant_id=$1 AND content_id='import-b'", schema), tenant).Scan(&dirty); err != nil || dirty != 0 {
		t.Fatalf("rollback queue: %d %v", dirty, err)
	}
	// An ordinary edit resumes the normal transaction clock, preserving creation.
	slug := "ordinary-edit"
	before := time.Now().Add(-time.Second)
	node, err := s.UpdateNode(ctx, "import-a", NodeUpdate{Slug: &slug})
	if err != nil {
		t.Fatal(err)
	}
