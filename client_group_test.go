...
func TestGroupByContentOrdersItemsWithoutEnglishPreference(t *testing.T) {
	...
	docs := []groupedDoc{
		{ref: v(cid(1), "a-en"), language: "en", score: .9},
		{ref: v(cid(2), "b-es"), language: "es", score: .9, requested: true},
		{ref: v(cid(1), "a-es"), language: "es", score: .5, requested: true, priority: 1},
		{ref: v(cid(1), "a-es2"), language: "es", score: .5, requested: true},
		{ref: v(cid(3), "c-en"), language: "en", score: 1},
		{ref: contentref.New("t", "w", cid(3)), language: "es", score: .9, requested: true},
	}
	var got []string
	...
	want := []string{cid(3) + ":c-en/en", cid(2) + ":b-es/es", cid(3) + ":/es", cid(1) + ":a-es2/es"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("groups=%v want %v", got, want)
	}
	groups := groupByContent(docs)
	if p, more := page(groups, 1, 2); len(p) != 2 || !more || p[0].best.ref.ContentID != cid(2) {
