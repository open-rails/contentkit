...
// when done.
//
//	DATABASE_URL                 host Postgres (holds WorkerSchema)
...
//
//	MEDIA_WORKER_RIVER_SCHEMA    the worker's River schema (default media_worker)
...
func (c *Config) TuningFromEnv() error {
	for k, p := range map[string]*string{"MEDIA_WORKER_RIVER_SCHEMA": &c.WorkerSchema, "MEDIA_HOST_RIVER_SCHEMA": &c.HostSchema, "MEDIA_HOST_QUEUE": &c.HostQueue, "MEDIA_WORKER_TMP": &c.TempDir,
