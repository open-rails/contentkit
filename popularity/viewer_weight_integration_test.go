...
func TestIntegrationViewerWeightedQuality(t *testing.T) {
	...
	for _, work := range []string{"base", "heavy", "heavy-late", "audience"} {
		shift := 0
		if work == "heavy-late" {
			shift = 20
		}
	...
	metrics := metricsByID(t, hub, []string{"base", "heavy", "heavy-late", "audience", "legacy"}, window)
	for id, want := range map[string]struct {
		views, returning uint64
		engagement       float64
		score            int64
	}{
		"base": {100, 0, 0, 0}, "heavy": {1099, 1, .999, 99900}, "heavy-late": {1099, 1, .999, 99900}, "audience": {200, 100, 50, 10000},
	} {
		m := metrics[id]
	...
	if scores["heavy"] != scores["heavy-late"] {
