...
CREATE TABLE content_search_backfill (
    content_kind text CONSTRAINT search_documents_backfill_state_entity_type_not_null NOT NULL,
    language text CONSTRAINT search_documents_backfill_state_language_not_null NOT NULL,
    cursor text DEFAULT ''::text CONSTRAINT search_documents_backfill_state_cursor_not_null NOT NULL,
    state text DEFAULT 'running'::text CONSTRAINT search_documents_backfill_state_state_not_null NOT NULL,
    last_error text,
    updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP CONSTRAINT search_documents_backfill_state_updated_at_not_null NOT NULL,
    tenant_id text NOT NULL
);

CREATE TABLE content_search_dirty (
    content_kind text CONSTRAINT search_dirty_entity_type_not_null NOT NULL,
    content_id text CONSTRAINT search_dirty_entity_id_not_null NOT NULL,
    language text CONSTRAINT search_dirty_language_not_null NOT NULL,
    is_deleted boolean DEFAULT false CONSTRAINT search_dirty_is_deleted_not_null NOT NULL,
    reason text DEFAULT 'unknown'::text CONSTRAINT search_dirty_reason_not_null NOT NULL,
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP CONSTRAINT search_dirty_created_at_not_null NOT NULL,
    updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP CONSTRAINT search_dirty_updated_at_not_null NOT NULL,
    revision bigint CONSTRAINT search_dirty_revision_not_null NOT NULL,
    tenant_id text NOT NULL,
    content_version_id text DEFAULT ''::text NOT NULL
);
...
CREATE TABLE content_search_documents (
    content_kind text CONSTRAINT search_documents_entity_type_not_null NOT NULL,
    content_id text CONSTRAINT search_documents_entity_id_not_null NOT NULL,
    language text CONSTRAINT search_documents_language_not_null NOT NULL,
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP CONSTRAINT search_documents_created_at_not_null NOT NULL,
    updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP CONSTRAINT search_documents_updated_at_not_null NOT NULL,
    raw_document text,
    title text DEFAULT ''::text CONSTRAINT search_documents_title_not_null NOT NULL,
    aliases text[] DEFAULT '{}'::text[] CONSTRAINT search_documents_aliases_not_null NOT NULL,
    keywords text[] DEFAULT '{}'::text[] CONSTRAINT search_documents_keywords_not_null NOT NULL,
    tenant_id text NOT NULL,
    content_version_id text DEFAULT ''::text NOT NULL
);
...
CREATE TABLE social_comments (
    ...
    content_kind text CONSTRAINT social_comments_entity_type_not_null NOT NULL,
    content_id text CONSTRAINT social_comments_entity_id_not_null NOT NULL,
    reply_to_id uuid,
    user_id text,
    anon_name text,
    body text NOT NULL,
    ...
    CONSTRAINT social_comments_moderation_ck CHECK ((moderation = ANY (ARRAY['approved'::text, 'held'::text, 'rejected'::text]))),
    CONSTRAINT social_comments_moderation_revision_check CHECK ((moderation_revision > 0))
);

CREATE TABLE social_entity_counts (
    content_kind text CONSTRAINT social_entity_counts_entity_type_not_null NOT NULL,
    content_id text CONSTRAINT social_entity_counts_entity_id_not_null NOT NULL,
    likes integer DEFAULT 0 NOT NULL,
    dislikes integer DEFAULT 0 NOT NULL,
...
CREATE TABLE social_favorites (
    ...
    content_kind text CONSTRAINT social_favorites_entity_type_not_null NOT NULL,
    content_id text CONSTRAINT social_favorites_entity_id_not_null NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    tenant_id text NOT NULL,
    content_version_id text DEFAULT ''::text NOT NULL
);
...
CREATE TABLE social_poll_answers (
    ...
    group_label text,
    CONSTRAINT social_poll_answers_revision_check CHECK ((revision > 0))
);
...
CREATE TABLE social_poll_questions (
    ...
    CONSTRAINT social_poll_questions_kind_ck CHECK ((kind = ANY (ARRAY['multiple_choice'::text, 'free_text'::text])))
);
...
CREATE TABLE social_posts (
    ...
    CONSTRAINT social_posts_moderation_ck CHECK ((moderation = ANY (ARRAY['approved'::text, 'held'::text, 'rejected'::text]))),
    CONSTRAINT social_posts_moderation_revision_check CHECK ((moderation_revision > 0))
);

CREATE TABLE social_reactions (
    content_kind text CONSTRAINT social_reactions_entity_type_not_null NOT NULL,
    content_id text CONSTRAINT social_reactions_entity_id_not_null NOT NULL,
    user_id text,
    ip text,
    value smallint NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    ...
    CONSTRAINT social_reactions_value_ck CHECK ((value = ANY (ARRAY['-1'::integer, 0, 1])))
);
...
ALTER TABLE ONLY content_private_subject_erasures
    ADD CONSTRAINT content_private_subject_erasures_pkey PRIMARY KEY (tenant_id, actor_id);
...
ALTER TABLE ONLY social_comments
    ADD CONSTRAINT social_comments_pkey PRIMARY KEY (id);

ALTER TABLE ONLY social_entity_counts
    ADD CONSTRAINT social_entity_counts_pkey PRIMARY KEY (tenant_id, content_kind, content_id, content_version_id);

ALTER TABLE ONLY social_favorites
    ADD CONSTRAINT social_favorites_pkey PRIMARY KEY (tenant_id, user_id, content_kind, content_id, content_version_id);

ALTER TABLE ONLY social_poll_answers
    ADD CONSTRAINT social_poll_answers_actor_uq UNIQUE (tenant_id, question_id, actor_id);

ALTER TABLE ONLY social_poll_answers
    ADD CONSTRAINT social_poll_answers_pkey PRIMARY KEY (id);

ALTER TABLE ONLY social_poll_options
    ADD CONSTRAINT social_poll_options_pkey PRIMARY KEY (id);

ALTER TABLE ONLY social_poll_questions
    ADD CONSTRAINT social_poll_questions_pkey PRIMARY KEY (id);

ALTER TABLE ONLY social_poll_votes
    ADD CONSTRAINT social_poll_votes_pkey PRIMARY KEY (id);

ALTER TABLE ONLY social_posts
...
CREATE INDEX social_comments_author_published_idx ON social_comments USING btree (tenant_id, user_id) INCLUDE (likes, dislikes) WHERE ((deleted_at IS NULL) AND (moderation = 'approved'::text) AND (user_id IS NOT NULL));

CREATE INDEX social_comments_held_idx ON social_comments USING btree (tenant_id, created_at, id) WHERE (moderation = 'held'::text);

CREATE INDEX social_comments_latest_idx ON social_comments USING btree (tenant_id, created_at DESC) WHERE (deleted_at IS NULL);

CREATE INDEX social_comments_reply_idx ON social_comments USING btree (reply_to_id, created_at) WHERE (reply_to_id IS NOT NULL);

CREATE INDEX social_comments_toplevel_idx ON social_comments USING btree (tenant_id, content_kind, content_id, content_version_id, created_at DESC) WHERE (reply_to_id IS NULL);

CREATE INDEX social_entity_counts_best_idx ON social_entity_counts USING btree (tenant_id, content_kind, (
CASE
...
CREATE INDEX social_entity_counts_comments_idx ON social_entity_counts USING btree (tenant_id, content_kind, comment_count DESC) WHERE (comment_count > 0);

CREATE INDEX social_entity_counts_favorites_idx ON social_entity_counts USING btree (tenant_id, content_kind, favorites DESC) WHERE (favorites > 0);

CREATE INDEX social_entity_counts_likes_idx ON social_entity_counts USING btree (tenant_id, content_kind, likes DESC) WHERE (likes > 0);

CREATE INDEX social_favorites_content_idx ON social_favorites USING btree (tenant_id, content_kind, content_id, content_version_id);

CREATE INDEX social_favorites_user_created_idx ON social_favorites USING btree (tenant_id, user_id, created_at DESC);

CREATE INDEX social_poll_answers_pending_idx ON social_poll_answers USING btree (tenant_id, id) WHERE (classified_at IS NULL);

CREATE INDEX social_poll_options_question_idx ON social_poll_options USING btree (question_id, "position");

CREATE INDEX social_poll_questions_live_idx ON social_poll_questions USING btree (tenant_id, language, live_at DESC) WHERE ((deleted_at IS NULL) AND is_active);

CREATE UNIQUE INDEX social_poll_votes_ip_uq ON social_poll_votes USING btree (question_id, ip) WHERE ((user_id IS NULL) AND (ip IS NOT NULL));

CREATE UNIQUE INDEX social_poll_votes_user_uq ON social_poll_votes USING btree (question_id, user_id) WHERE (user_id IS NOT NULL);

CREATE INDEX social_posts_held_idx ON social_posts USING btree (tenant_id, created_at, id) WHERE (moderation = 'held'::text);

CREATE INDEX social_posts_published_idx ON social_posts USING btree (tenant_id, language, live_at DESC) WHERE ((deleted_at IS NULL) AND (is_draft = false));

CREATE UNIQUE INDEX social_posts_slug_uq ON social_posts USING btree (tenant_id, slug) WHERE ((slug IS NOT NULL) AND (deleted_at IS NULL));

CREATE INDEX social_reactions_content_idx ON social_reactions USING btree (tenant_id, content_kind, content_id, content_version_id);

CREATE UNIQUE INDEX social_reactions_ip_uq ON social_reactions USING btree (tenant_id, content_kind, content_id, content_version_id, ip) WHERE ((user_id IS NULL) AND (ip IS NOT NULL));

CREATE UNIQUE INDEX social_reactions_user_uq ON social_reactions USING btree (tenant_id, content_kind, content_id, content_version_id, user_id) WHERE (user_id IS NOT NULL);
...
ALTER TABLE ONLY social_comments
    ADD CONSTRAINT social_comments_reply_to_id_fkey FOREIGN KEY (reply_to_id) REFERENCES social_comments(id) ON DELETE CASCADE;

ALTER TABLE ONLY social_poll_answers
    ADD CONSTRAINT social_poll_answers_question_id_fkey FOREIGN KEY (question_id) REFERENCES social_poll_questions(id) ON DELETE CASCADE;

ALTER TABLE ONLY social_poll_options
    ADD CONSTRAINT social_poll_options_question_id_fkey FOREIGN KEY (question_id) REFERENCES social_poll_questions(id) ON DELETE CASCADE;

ALTER TABLE ONLY social_poll_votes
    ADD CONSTRAINT social_poll_votes_option_id_fkey FOREIGN KEY (option_id) REFERENCES social_poll_options(id) ON DELETE CASCADE;

ALTER TABLE ONLY social_poll_votes
    ADD CONSTRAINT social_poll_votes_question_id_fkey FOREIGN KEY (question_id) REFERENCES social_poll_questions(id) ON DELETE CASCADE;
